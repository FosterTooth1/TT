...
for c = 1:num_combinations
    ...
    [mejor, media, peor, desviacion, tiempo] = EjecutarAlgoritmo(Num_Pob, Num_Gen, Pm, m, Num_Competidores, Hijos_Crossover, Distancias, Num_iteraciones);

    % Guardar resultados
    Resultados_GridSearch = [Resultados_GridSearch; {Num_Pob, Num_Gen, Pm, m, Num_Competidores, Hijos_Crossover, mejor, media, peor, desviacion, tiempo}];
end

% Convertir resultados a una tabla
Resultados_GridSearch = cell2table(Resultados_GridSearch, ...
    'VariableNames', {'Num_Pob', 'Num_Gen', 'Pm', 'm', 'Num_Competidores', 'Hijos_Crossover', ...
    'Mejor', 'Media', 'Peor', 'Desviacion', 'Tiempo'});

% Mostrar la mejor configuración
[~, idx] = min(Resultados_GridSearch.Mejor);
mejor_config = Resultados_GridSearch(idx, :);
disp('Mejor configuración encontrada:');
disp(mejor_config);

% Graficar los resultados
figure;
plot(1:height(Resultados_GridSearch), Resultados_GridSearch.Mejor, '-o');
xlabel('Configuración de Parámetros');
ylabel('Mejor Aptitud');
title('Grid Search: Mejor Aptitud por Configuración');
grid on;

figure;
plot(1:height(Resultados_GridSearch), Resultados_GridSearch.Tiempo, '-x');
xlabel('Configuración de Parámetros');
ylabel('Tiempo de Ejecución (s)');
title('Grid Search: Tiempo de Ejecución por Configuración');
grid on;

% Función principal del algoritmo genético
function [mejor, media, peor, desviacion, tiempo] = EjecutarAlgoritmo(Num_Pob, Num_Gen, Pm, m, Num_Competidores, Hijos_Crossover, Distancias, Num_iteraciones)
...
for iteracion = 1:Num_iteraciones
    ...
    while iter <= Num_Gen
        ...
        Padres = SeleccionTorneo(Pob, Aptitud_Pob, Num_Pob, Num_Competidores);
        ...
        for i = 1:2:Num_Pob
            if Hijos_Crossover == 1
                ...
                individuos = [Padres(i, :); Padres(i+1, :); hijo_1];
                aptitudes = [Aptitud_Padre_1; Aptitud_Padre_2; Aptitud_Hijo_1];
...
function Padres = SeleccionTorneo(Pob, Aptitud_Pob, Num_Pob, Num_Competidores)
