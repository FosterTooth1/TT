...
void evaluar_poblacion(poblacion *poblacion, double **distancias, int longitud_genotipo, double ventanas_tiempo[][2]) {
    ...
    for (int i = 0; i < poblacion->tamano; i++) {
        poblacion->individuos[i].fitness = evaluar_individuo(poblacion->individuos[i].genotipo, distancias, longitud_genotipo, ventanas_tiempo);
    }
}

// Función para calcular la distancia total recorrida por el individuo (fitness)
// Recibe un genotipo, una matriz de distancias, la longitud del genotipo y las ventanas de tiempo
// Devuelve el fitness del individuo
double evaluar_individuo(int *genotipo, double **distancias, int longitud_genotipo, double ventanas_tiempo[][2]) {
    double total_cost = 0.0;      // Costo total del recorrido (en horas)
    double tiempo_acumulado = 0; // Tiempo transcurrido desde el inicio del recorrido

    for (int i = 0; i < longitud_genotipo; i++) {
        int origen = genotipo[i];
        int destino = genotipo[(i + 1) % longitud_genotipo]; // Circular: última ciudad regresa a la primera

        // Añadir el tiempo de viaje entre ciudades
        double tiempo_viaje = distancias[origen][destino];
        tiempo_acumulado += tiempo_viaje;

        // Calcular la hora de llegada al destino (ajustando al formato de 24 horas)
        double hora_llegada = fmod(tiempo_acumulado, 24);

        // Ventanas de tiempo de la ciudad de destino
        double ventana_inicio = ventanas_tiempo[destino][0];
        double ventana_fin = ventanas_tiempo[destino][1];

        // Ajustar el tiempo acumulado si se llega fuera de la ventana permitida
        if (hora_llegada < ventana_inicio) {
            tiempo_acumulado += (ventana_inicio - hora_llegada); // Esperar hasta el inicio de la ventana
        } else if (ventana_fin < ventana_inicio) { 
            // Caso especial: ventana cruza medianoche (ej. 22:00 a 02:00)
            if (hora_llegada > ventana_fin && hora_llegada < ventana_inicio) {
                tiempo_acumulado += (24 - hora_llegada + ventana_inicio); // Esperar al siguiente día
            }
...
void cruzar_individuos(poblacion *padres, poblacion *hijos, int num_pob, int longitud_genotipo, int m, double **distancias, double probabilidad_cruce, double ventanas_tiempo[][2]) {
    for (int i = 0; i < num_pob; i += 2) {
        ...
        if ((double)rand() / RAND_MAX < probabilidad_cruce) {
            ...
            heuristica_abruptos(hijo1, longitud_genotipo, m, distancias, ventanas_tiempo);
            heuristica_abruptos(hijo2, longitud_genotipo, m, distancias, ventanas_tiempo);
            ...
            for (int j = 0; j < 4; j++) {
                ...
                temp_hijos[j].fitness = evaluar_individuo(individuos[j], distancias, longitud_genotipo, ventanas_tiempo);
            }
...
void mutar_individuo(individuo *individuo, double **distancias, double probabilidad_mutacion, int longitud_genotipo, double ventanas_tiempo[][2]) {
    ...
    if ((double)rand() / RAND_MAX < probabilidad_mutacion) {
        ...
        individuo->fitness = evaluar_individuo(individuo->genotipo, distancias, longitud_genotipo, ventanas_tiempo);
    }
}
...
void heuristica_abruptos(int* ruta, int num_ciudades, int m, double** distancias, double ventanas_tiempo[][2]) {
    ...
    for (int i = 0; i < num_ciudades; i++) {
        ...
        double mejor_costo = evaluar_individuo(ruta, distancias, num_ciudades, ventanas_tiempo);
        ...
        for (int j = 1; j <= m && j < num_ciudades; j++) {
            ...
            if (pos_cercana != -1) {
                ...
                for (int posicion_antes_o_despues = 0; posicion_antes_o_despues <= 1; posicion_antes_o_despues++) {
                    ...
                    double nuevo_costo = evaluar_individuo(ruta_temp, distancias, num_ciudades, ventanas_tiempo);
